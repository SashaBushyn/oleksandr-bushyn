...
public class BookingRepositoryImpl implements BookingRepository {
    ...
    public Booking updateBooking(Long id, Booking booking) {
        boolean isDeleted = bookingList.removeIf(u -> u.getId().equals(id));
        if (isDeleted) {
            bookingList.add(booking);
        } else {
            throw new EntityException("booking with id "+ id +" is not found");
        }
        return booking;
    }
    ...
    public void deleteBooking(Long id) {
        bookingList.removeIf(booking -> booking.getId().equals(id));
    }
}
